...
const encode = (data = '', config = {escSeq: '01111110', escNum: 5}) => {
    ...
    //WE DID IT REDDIT
    for (let c of data) outputData += c + (((oneCounter = (oneCounter + (c === '1'))) === config.escNum) ? ((oneCounter = 0) === 0 ? '0': 'this') : '');
...
const decode = (data = '', config = {escSeq: '01111110', escNum: 5}) => {
    ...
    for (let c of data) outputData += ((oneCounter = (oneCounter + (c === '1'))) === config.escNum) ? ((delFlag = true) === true && (oneCounter -= config.escNum) === 0 ? c : 'is a') : (delFlag ? ((delFlag = false) === false ? '' : 'joke code') : c);
